...
function MetiersBlocs(){
    ...
    const [currentIndices, setCurrentIndices] = useState(rds.map(() => 0));
                ...
                <div key={rd.id} className='relative w-full'>
                    ...
                    <div className="flex flex-col items-center justify-center align-center text-center text-white pb-10 pt-20">
                        ...
                        <div className="absolute">
                            <h2 className="text-4xl pb-5 md:text-4xl lg:text-6xl 2xl:text-6xl 3xl:text-7xl lg:pb-10 xl:pb-16 bold font-primary">{rd.header.title}</h2>
                            <hr className="w-1/3 m-auto border-1 border-white"></hr>
                            <p className="text-xl pt-5 w-5/6  md:text-lg lg:text-xl xl:text-2xl 3xl:text-3xl lg:pt-10 xl:pt-16 sm:w-2/3 m-auto font-primary bold italic">{rd.header.description} </p>
                        </div>
                    </div>
                    <div className='grid grid-cols-1 sm:grid-cols-2 items-center'>
                        {/* Texte */}
                        <div>
                            {rd.contenu.liste && rd.contenu.liste.length > 0 && (
                                <div className="flex-col sm:flex-row flex w-2/3 m-auto">
                                    <div className="w-full pb-10 sm:pb-10 sm:pr-10">
                                        <h2 className="font-primary text-primary mb-5 text-2xl 2xl:text-3xl 3xl:text-4xl">{rd.contenu.titre}</h2>
                                        <ul className="custom-bullet-list text-secondary text-lg 3xl:text-xl">
                                            {rd.contenu.liste.map(item => <li>{item}</li>)}
                                        </ul>
                                    </div>
                                </div>
                            )}
                            {/* Video */}
                            {rd.video.titre && rd.video.titre.length > 0 && (
                                <NavLink to={rd.video.lien} className="flex-col sm:flex-row flex w-2/3 m-auto items-center">
                                        <img src="../images/RD/play_video.svg" alt="placeholder" className="max-h-10 mr-2"/>
                                        <p className='text-primary'>{rd.video.titre}</p>
                                </NavLink>
                            )}
                        </div>
                        <div className='m-auto pt-16'>
                            {/* Visuel */}
                            {rd.visuel && rd.visuel.src && rd.visuel.bottom && ( 
                                <div style={{ marginBottom : rd.visuel.bottom }} className="-z-inde-10">
                                    <img src={rd.visuel.src} alt="placeholder" className='max-w-[75%]'/>
                                </div>
                            )}
                        </div>
                    ...
                    {/* Encart */}
                    {rd.encart && rd.encart.length &&(
                        <div className="w-2/3 m-auto sm:pl-5 border-2 rounded-3xl p-5 mt-10">
                            <p>{rd.encart}</p>
                        </div>
                    )}
                    {/* carousel */}
                    {rd.carousel.imgs && rd.carousel.imgs.length > 0 && (
                        <div id="carousel" className="m-auto py-32 flex flex-row justify-center items-center">
                            {/* Fléche gauche */}
                            <div 
